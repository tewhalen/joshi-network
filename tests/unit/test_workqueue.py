...
def test_deduplication():
    ...
    queue.enqueue(
        WorkItem(priority=5, wrestler_id=100, operation="refresh_profile")
    )  # Duplicate - should be ignored

    # Add same wrestler with different operation
    queue.enqueue(
        WorkItem(priority=10, wrestler_id=100, operation="refresh_matches", year=2025)
    )
...
def test_workitem_with_year():
    ...
    queue.enqueue(
        WorkItem(priority=10, wrestler_id=1, operation="refresh_matches", year=2025)
    )
    queue.enqueue(
        WorkItem(priority=10, wrestler_id=1, operation="refresh_matches", year=2024)
    )
