...
def test_priority_ordering():
    ...
    queue.enqueue(WorkItem(priority=30, wrestler_id=1, operation="test"))
    queue.enqueue(WorkItem(priority=1, wrestler_id=2, operation="test"))
    queue.enqueue(WorkItem(priority=50, wrestler_id=3, operation="test"))
    queue.enqueue(WorkItem(priority=10, wrestler_id=4, operation="test"))
    queue.enqueue(WorkItem(priority=5, wrestler_id=5, operation="test"))
...
def test_deduplication():
    ...
    queue.enqueue(WorkItem(priority=10, wrestler_id=100, operation="refresh_profile"))
    queue.enqueue(WorkItem(priority=5, wrestler_id=100, operation="refresh_profile"))  # Duplicate - should be ignored
    
    # Add same wrestler with different operation
    queue.enqueue(WorkItem(priority=10, wrestler_id=100, operation="refresh_matches", year=2025))
...
def test_sentinel_filtering():
    ...
    queue.enqueue(WorkItem(priority=1, wrestler_id=-1, operation="test"))
    
    # Add valid item
    queue.enqueue(WorkItem(priority=1, wrestler_id=200, operation="test"))
    
    # Should only have 1 item (sentinel filtered out)
    assert len(queue) == 1
    
    item = queue.dequeue()
    assert item.wrestler_id == 200
...
def test_workitem_with_year():
    ...
    queue.enqueue(WorkItem(priority=10, wrestler_id=1, operation="refresh_matches", year=2025))
    queue.enqueue(WorkItem(priority=10, wrestler_id=1, operation="refresh_matches", year=2024))
